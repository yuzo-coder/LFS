...
mkdir -p "$TARGET_HOME/.config/wlogout"
mkdir -p "$TARGET_HOME/.config/wofi"
mkdir -p "$TARGET_HOME/.config/yazi"
...
cat > "$TARGET_HOME/.config/sway/foot/foot.ini" << 'EOF'
[main]
...
cat > "$TARGET_HOME/.config/sway/waybar/config.json" << 'EOF'
// -*- mode: jsonc -*-
{
    "layer": "top",
    "position": "top", 
...
cat > "$TARGET_HOME/.config/sway/waybar/style.css" << 'EOF'
...
cat > "$TARGET_HOME/.config/sway/wlogout/style.css" << 'EOF'
window {
...
button {
    ...
    /* --- ここから文字の設定を追加 --- */
    color: #ffffff;              /* 文字を真っ白にする */
    font-size: 20px;             /* 文字を大きくする（お好みで調整） */
    font-weight: bold;           /* 太字にする */
    text-shadow: 0 0 5px rgba(0,0,0,1); /* 文字の周りに黒い影をつけてクッキリさせる */
    
    /* 文字の位置をアイコンの下側に調整 */
    background-origin: padding-box;
    background-clip: padding-box;
    padding-top: 100px;         /* アイコンの下に文字が来るように押し下げる */
}
...
/* 重要：各ボタンのアイコン画像を再定義（パスが通っているか確認） */
#lock {
...
cat > "$TARGET_HOME/.config/sway/wofi/config" << 'EOF'
show=drun
width=400
height=500
...
cat > "$TARGET_HOME/.config/sway/wofi/style.css" << 'EOF'
window {
    margin: 5px;
    border: 2px solid #ffffff;
