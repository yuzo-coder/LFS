...
If the I<file> is a local file, on systems that support it, it will be
opened with the C<O_NONBLOCK> flag to allow non-blocking reads from the
file. B<Wget> will attempt to continue reading from the file until
